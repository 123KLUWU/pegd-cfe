{{-- resources/views/admin/generated_documents/show.blade.php --}}
@extends('layouts.app')

@section('title', 'Detalles de Documento Generado: ' . $generatedDocument->title)

@section('content')
<div class="container mt-5">
    <a href="{{ route('generated-documents.index') }}" class="btn btn-secondary mb-3">← Volver a Documentos Generados</a>
        ...
        <div class="col-md-6">
            <div class="card mb-4">
                ...
                <ul class="list-group list-group-flush">
                    ...
                    <li class="list-group-item"><strong>Visibilidad:</strong> {{ ucfirst(str_replace('_', ' ', $generatedDocument->visibility_status)) }}
                        <form action="{{ route('admin.generated-documents.change_visibility', $generatedDocument->id) }}" method="POST" class="d-inline ms-2">
                            @csrf
                            <select name="visibility_status" class="form-select form-select-sm d-inline-block w-auto" onchange="this.form.submit()">
                                <option value="public_editable" {{ $generatedDocument->visibility_status == 'public_editable' ? 'selected' : '' }}>P. Editable</option>
                                <option value="public_viewable" {{ $generatedDocument->visibility_status == 'public_viewable' ? 'selected' : '' }}>P. Ver</option>
                                <option value="private_restricted" {{ $generatedDocument->visibility_status == 'private_restricted' ? 'selected' : '' }}>Privado</option>
                            </select>
                        </form>
                    </li>
            ...
            <div class="mb-3">
                ...
                @if (!$generatedDocument->trashed())
                    <form action="{{ route('generated-documents.destroy', $generatedDocument->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                ...
                @else
                    <form action="{{ route('admin.generated-documents.restore', $generatedDocument->id) }}" method="POST" class="d-inline me-2">
                        @csrf
                    ...
                    <form action="{{ route('admin.generated-documents.force_delete', $generatedDocument->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-dark" onclick="return confirm('¡ADVERTENCIA! ¿Estás seguro de eliminar PERMANENTEMENTE este documento de la DB Y Google Drive? Esta acción es irreversible.')">Borrar Perm.</button>
                    </form>
                @endif
            </div>
        </div>
